...
static std::string ParsePqcapMetadataLocation(FileHandle &handle, const std::string &source) {
	...
	handle.Read(tail.data(), PQCAP_FOOTER_BLOCK_SIZE, file_size - PQCAP_FOOTER_BLOCK_SIZE);

	pqcap_reader::FooterLocator loc;
	if (!pqcap_reader::ParseFooterLocator(std::string(tail.data(), tail.size()), loc)) {
		throw InvalidInputException("invalid pqcap footer locator: " + source);
	}
	if (loc.parquet_length == 0) {
		throw InvalidInputException("pqcap embedded metadata length is zero: " + source);
	}
	if (loc.parquet_offset + loc.parquet_length < loc.parquet_offset ||
	    loc.parquet_offset + loc.parquet_length > file_size) {
		throw InvalidInputException("pqcap footer offset/length out of range: " + source);
	}
	return std::to_string(loc.parquet_offset) + "_" + std::to_string(loc.parquet_length);
}
...
static void PqcapOffsetSizeFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	auto &fs = FileSystem::GetFileSystem(state.GetContext());
	StringScalarLoop(args, result, "pqcap_offset_size", [&](const std::string &path) {
...
static void LoadInternal(ExtensionLoader &loader) {
	...
	ScalarFunction offset_size_fn("pqcap_offset_size", {LogicalType::VARCHAR}, LogicalType::VARCHAR,
	                              PqcapOffsetSizeFunction);
