...
public:
  PqcapSubFileHandle(FileSystem &file_system, const string &path,
                     FileOpenFlags flags, unique_ptr<FileHandle> inner_handle_p,
                     idx_t base_offset_p, idx_t sub_size_p)
      : FileHandle(file_system, path, flags),
        inner_handle(std::move(inner_handle_p)), base_offset(base_offset_p),
...
public:
  ...
  std::string GetName() const override { return "PqcapSubFileSystem"; }
  ...
  bool FileExists(const string &filename,
                  optional_ptr<FileOpener> opener) override {
    return filename.size() >= 16 &&
           filename.substr(0, 16) == "pqcap-subfile://";
  }
};
